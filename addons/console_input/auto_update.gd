...
@export var watch_internal_props: bool = false
@export var watch_invis: bool = false
...
func _process(delta: float):
	if Engine.is_editor_hint() and auto_update and (get("visible") or watch_invis) and eval_check(delta):
		eval_dump()
...
func eval_dump():
	...
	for prop in eval_obj.get_property_list():
		if not watch_internal_props and not is_export (prop.usage ) :
			continue
		var key = prop.name
